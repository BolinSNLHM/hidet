...
class MatmulF32Taskx86(Task):
    ...
    def schedule_matmulf32_x86(
        ...
        from hidet.lang.cpu import avx_f32x8_load_aligned, avx_f32x8_store_aligned
        ...
        with hidet.script_module() as module:
            ...
            def matmul_kernel_x86(a: float32[m_size, k_size], b: float32[k_size, n_size], c: float32[m_size, n_size]):
                ...
                packed_a_alloc = avx_malloc(aip_outer_rows * tile_m * block_k * 4, 64)
                packed_b_alloc = avx_malloc(bip_outer_cols * block_k * tile_n * 4, 64)
                        ...
                        for nb in range(nbs):
                            ...
                            for micropanel_idx in range(np):
                                ...
                                for micropanel_row_iter in range(micropanel_row_iters):
                                    ...
                                    avx_f32x8_store_aligned(~packed_b[br0, panel_col_start], b00)
                                    avx_f32x8_store_aligned(~packed_b[br0, panel_col_start + 8], b08)
                                    avx_f32x8_store_aligned(~packed_b[br1, panel_col_start], b10)
                                    avx_f32x8_store_aligned(~packed_b[br1, panel_col_start + 8], b18)
                                    avx_f32x8_store_aligned(~packed_b[br2, panel_col_start], b20)
                                    avx_f32x8_store_aligned(~packed_b[br2, panel_col_start + 8], b28)
                                    avx_f32x8_store_aligned(~packed_b[br3, panel_col_start], b30)
                                    avx_f32x8_store_aligned(~packed_b[br3, panel_col_start + 8], b38)
                                ...
                                if micropanel_row_iter_remain > 0:
                                    ...
                                    for remaining_row in range(micropanel_row_iter_remain):
                                        b0 = avx_f32x8_load(
                                            ~b[p + remaining_row_start + remaining_row, j + panel_col_start])
                                        b1 = avx_f32x8_load(
